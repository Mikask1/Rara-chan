...
class Instagram_(commands.Cog, name="Instagram"):
    ...
    def _get_Carousel_embed(self, username: str, posts: Post, start_time: float) -> DropdownPaginator:
        ...
        for n, media in enumerate(posts.media, start=1):
            ...
            if media.media_type == "image":
                embed = self._get_Media_embed(
                    post=posts, username=username, media=media, start_time=start_time)
    ...
    async def get(self, ctx: discord.abc.Messageable, option, *, query: str):
        ...
        post = await task
        if post.is_carousel:
            paginator = self._get_Carousel_embed(
                posts=post, username=profile.username, start_time=start_time)
            await loading.delete()
            await paginator.send(ctx)
        else:
            embed = self._get_Media_embed(
                post=post, username=profile.username, media=post.media, start_time=start_time)
