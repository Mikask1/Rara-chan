...
class NHentai_(commands.Cog, name="NHentai"):
    ...
    def _get_doujin_embed(self, doujin):
        ...
        if len(doujin.parodies) != 0:
            embed.add_field(name="Parodies", value=", ".join(
                doujin.parodies), inline=False)

        if len(doujin.characters) != 0:
            embed.add_field(name="Characters", value=", ".join(
                doujin.characters), inline=False)

        if len(doujin.tags) != 0:
            embed.add_field(name="Tags", value=", ".join(
                doujin.tags), inline=False)

        if len(doujin.artists) != 0:
            embed.add_field(name="Authors", value=", ".join(
                doujin.artists), inline=True)

        if len(doujin.groups) != 0:
            embed.add_field(name="Groups", value=", ".join(
                doujin.groups), inline=True)
    ...
    async def search(self, ctx, *, query: str):
        ...
        if len(doujins):  # Check if there are any results
            ...
            paginator = DropdownPaginator(
                page_list, "Choose Page..", options=options)

            paginator.customize_button(
                "next", button_label=">", button_style=discord.ButtonStyle.green)
            paginator.customize_button(
                "prev", button_label="<", button_style=discord.ButtonStyle.green)
            paginator.customize_button(
                "first", button_label="<<", button_style=discord.ButtonStyle.blurple)
            paginator.customize_button(
                "last", button_label=">>", button_style=discord.ButtonStyle.blurple)
