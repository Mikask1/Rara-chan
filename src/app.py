...
SEGS = ["SEEEGGGGGGSSS", "SEEEEEEEEGGGGGSSSS", "SEGGGGS", "AHHHH KIMOCHI SEGGGGGGGGGGS", "ORE WA SEGGGGS DAAA", "YAMERO TAKESHI, OMAE NO KARADA GA SEGGS DEKINAI",
        "IYAAA SEGGGS NI NARITAI, SEGGGGGGGGSSSS", "ORE WA ZETTAI NI SEEGGS NI NARUDA", "SEKKKUSU NI NARITAI", "ORE NO CHINCHIN GA FURUERU", "Betsuni SEGGGSSS suki tte wake janai ndakara, baka. BAAKAAAAA~~"]
GREETINGS = ["hewwo~", "H-hi", "H-hello", "Wassup ma boi", "What is it darling?", "Konnichiwa~", "Sekkusu shiyou ze", "Domoo~",
             "Iyaa~~ hanashikakenaide", "Kimoi", "Shine", "Baka", "Betsuni sukitte wake janai ndakara, baka", "...", "yada~ hanashitakunai", "urusai baka"]
...
class RaraChan(commands.Bot):
    ...
    async def on_message(self, ctx: str):
        if not ctx.author.bot:
            ...
            if "damn" in pp_message:
                damn = "d"+"a" * \
                    randrange(1, 5)+"y"*randrange(1, 10)+"u" * \
                    randrange(1, 10)+"m"*randrange(1, 10)
                print(damn)
    ...
    async def on_command(self, ctx):
        ...
        with open("logs.txt", "a", encoding="utf-8") as log:
            log.write(
                f"{date}: User {user} used ?{command} in #{channel} at [{server}]\n")
...
class RaraHelpCommand(commands.DefaultHelpCommand):
    ...
    def set_information(self, embed):
        ...
        embed.add_field(
            name="​​", value="﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏﹏", inline=False)
        embed.set_footer(
    ...
    async def send_bot_help(self, mapping):
        ...
        paginator = DropdownPaginator(
            pages=page_list, options=options, placeholder="Choose category..")
    ...
    async def send_group_help(self, group):
        embed = discord.Embed(
            title=group.qualified_name.upper(), color=0x0080ff)

        embed.add_field(name="?"+group.qualified_name+"  " +
                        group.signature, value=group.short_doc, inline=False)

        subcommands = "\n?".join(
            [command.qualified_name for command in group.commands])
    ...
    async def send_command_help(self, command):
        embed = discord.Embed(
            title=command.qualified_name.upper(), color=0x0080ff)

        embed.add_field(name="?"+command.qualified_name+"  " +
                        command.signature, value="​​", inline=False)
